...
uint1 MQTT_PacketSaveData(const int8 *project_id,const char *device_name ,int16 send_len, int8 *type_bin_head, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	char topic_buf[48];
	
	snprintf(topic_buf, sizeof(topic_buf), "$sys/%s/%s/thing/property/post", project_id, device_name);
